...
if __name__ == "__main__":
    ...
    # blender_render_cmd = f'export PATH="/mnt/kostas-graid/sw/envs/vlongle/blender/blender-4.3.2-linux-x64:$PATH"; blender --background --python generate_blendernerf_data.py -- --obj_id {args.obj_id} --num_images {args.num_images} --format NGP --camera_dist 1.8 --output_dir /mnt/kostas-graid/datasets/vlongle/diffphys3d/data'
    blender_render_cmd = f'export PATH="/mnt/kostas-graid/sw/envs/vlongle/blender/blender-4.3.2-linux-x64:$PATH"; blender --background --python generate_blendernerf_data.py -- --obj_id {args.obj_id} --num_images {args.num_images} --format NGP --camera_dist 1.8 --output_dir /mnt/kostas-graid/datasets/vlongle/diffphys3d/data'
    convert_cmd = f"python convert.py --obj_id {args.obj_id}"
    train_cmd = f"ns-train f3rm --data /mnt/kostas-graid/datasets/vlongle/diffphys3d/data/{args.obj_id} --max-num-iterations {args.train_steps} --viewer.quit-on-train-completion True"
