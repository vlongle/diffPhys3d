...
def visualize_sphere(self, context):
    ...
    if EMPTY_NAME not in scene.objects.keys() and not scene.sphere_exists:
        ...
        # Use average of min and max for visualization
        scene.sphere_radius = (scene.sphere_radius_min + scene.sphere_radius_max) / 2
...
def sample_from_sphere(scene):
    ...
    # Sample a random radius between min and max
    random_radius = scene.sphere_radius_min + rng.random() * (scene.sphere_radius_max - scene.sphere_radius_min)
    
    # ellipsoid sample : center + rotation @ radius * unit sphere
    point = random_radius * mathutils.Vector(scene.sphere_scale) * unit
    rot_mat = mathutils.Euler(scene.sphere_rotation).to_matrix()
    point = mathutils.Vector(scene.sphere_location) + rot_mat @ point

    # invert direction if outwards
    if scene.outwards:
        direction = point - mathutils.Vector(scene.sphere_location)
        point = mathutils.Vector(scene.sphere_location) - direction
...
def properties_desgraph(scene):
    if scene.show_sphere and EMPTY_NAME in scene.objects.keys():
        ...
        # Update sphere_radius for visualization
